createQualificationReport <- function(
  ...
  findRepoRoot <- function(path) {
    ...
    repeat {
      if (
        file.exists(file.path(current, "Ketoconazole-Model.json")) &&
      ...
      if (identical(parent, current)) {
        stop(
          "Could not find repository root containing Ketoconazole-Model.json and Evaluation",
          call. = FALSE
        )
      }
      current <- parent
  ...
  snapshotFile <- file.path(repoRoot, "Ketoconazole-Model.json")
  ...
  if (!is.null(maxSimulationsPerCore)) {
    if (
      ...
      stop(
        "maxSimulationsPerCore must be a single integer value >= 1.",
        call. = FALSE
      )
    }
