createQualificationReport <- function(qualificationRunnerFolder,
  ...
  findRepoRoot <- function(path) {
    ...
    repeat {
      if (file.exists(file.path(current, "Ketoconazole-Model.json")) &&
          dir.exists(file.path(current, "Evaluation"))) {
        return(current)
      }
      parent <- dirname(current)
      if (identical(parent, current)) {
        stop("Could not find repository root containing Ketoconazole-Model.json and Evaluation", call. = FALSE)
      }
      current <- parent
    }
  }
